...
public class BookManagement implements Manageable {
    ...
    public void dislay() {
        if (mapBook.isEmpty()) {
            ...
            System.out.printf("┃%-5s|%-30s|%-15s|%-20s|%-20s|%-25s|%-15s┃\n", "ID", "TITLE", "AUTHOR", "PUBLICATION_YEAR", "PUBLISHER", "ISBN", "ACTIVE_BOOK");
            System.out.printf("┃%-5s|%-30s|%-15s|%-20s|%-20s|%-25s|%-15s┃\n", repeat("-", 5), repeat("-", 30), repeat("-", 15), repeat("-", 20), repeat("-", 20), repeat("-", 25), repeat("-", 15));
